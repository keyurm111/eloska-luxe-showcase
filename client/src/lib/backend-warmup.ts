...
class BackendWarmup {
  ...
  private readonly API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5004/api';
  ...
  // Initial warmup when website loads
  ...
  async checkBackendHealth(): Promise<BackendStatus> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('%c✅ Backend is healthy!', 'color: #10B981; font-weight: bold; font-size: 14px;');
      ...
      console.warn('%c⚠️ Backend health check failed!', 'color: #F59E0B; font-weight: bold; font-size: 14px;');
  ...
  startPeriodicChecks(): void {
    ...
    console.log(`⏰ Started periodic health checks every ${this.CHECK_INTERVAL / 1000} seconds`);
  }
  ...
  async warmupEndpoints(): Promise<void> {
    if (!this.isBackendReady()) {
      console.log('🔄 Backend not ready, warming up first...');
      await this.checkBackendHealth();
    }
    ...
    if (this.isBackendReady()) {
      ...
      console.warn('⚠️ Backend is not ready, API calls may fail');
    }
  }
}
...
if (typeof window !== 'undefined') {
  console.log('🌐 Backend warmup initialized for browser environment');
  warmupBackend();
}
